...
def navier_stokes_loss(model, x, y, mu, lambda_momentum=0.3, lambda_continuity=0.2):
...
"""	
def boundary_condition_loss(model, x_b, y_b, u_b, v_b):
    uvp_b = model(torch.cat((x_b, y_b), dim=1))
    u_b_pred = uvp_b[:, 0:1]
    v_b_pred = uvp_b[:, 1:2]
    ...
    return loss_u_b + loss_v_b
"""
def total_loss(model, x, y, u_exact, v_exact, mu, 
               lambda_momentum=0.2, lambda_continuity=0.1, lambda_data=1.0, lambda_bc=1.0, 
               p_exact=None, x_b = None, y_b= None, u_b = None, v_b = None):
    ...
    loss_bc = boundary_condition_loss(model, x_b, y_b, u_b, v_b) * lambda_bc if x_b is not None else 0
...
bc_data = pd.read_csv('bc_Data_PINN_Cavity_heatFlux.csv')
x_b = torch.tensor(bc_data['x'], dtype=torch.float32).to(device)
y_b = torch.tensor(bc_data['y'], dtype=torch.float32).to(device)
u_b = torch.tensor(bc_data['u'], dtype=torch.float32).to(device)  # Boundary u-values
v_b = torch.tensor(bc_data['v'], dtype=torch.float32).to(device)  # Boundary v-values
